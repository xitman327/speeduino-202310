...
class MCP_CAN
{
    ...
    INT8U   MCPINT;                                                     // Chip INT pin number
...
public:
    MCP_CAN(INT8U _CS, INT8U _INT = -1);
    ...
    INT8U write(CAN_message_t &CAN_tx_msg, bool sendMB = false){
      return sendMsgBuf(CAN_tx_msg.id, CAN_tx_msg.len, CAN_tx_msg.buf);
    }
    INT8U read(CAN_message_t &CAN_rx_msg){
      INT8U ret = 0;
      if(checkReceive() == CAN_MSGAVAIL){
        if(readMsgBuf(&CAN_rx_msg.id, &CAN_rx_msg.len, CAN_rx_msg.buf) == CAN_OK){
          ret = 1;
        }
      }
      return ret;
